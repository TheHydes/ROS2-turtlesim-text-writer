...
from my_robot_messages import Turtle, TurtleArray, CatchTurtleRequest, CatchTurtleResponse  # Import the message and service types

class HunterTurtle(Node):
    def __init__(self):
        super().__init__("hunter_turtle")
    ...
    def call_catch_turtle_server(self, turtle_name):
        client = self.create_client(CatchTurtleRequest, "catch_turtle")
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for Server Catch Turtle...")

        request = CatchTurtleRequest()
